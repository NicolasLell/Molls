...
public interface StockRepositorio extends CrudRepository<Stock, Long> {
    ...
    @Query(value = "SELECT stock.* FROM stock INNER JOIN producto p ON (stock.producto_id_producto = p.id_producto AND p.genero_producto_id_genero = 1) WHERE talle_producto_id_talle = :idMarca GROUP BY producto_id_producto", nativeQuery = true)
    public List<Stock> buscarMarcasHombres(@Param ("idMarca") Long idMarca);
    
    @Query(value = "SELECT stock.* FROM stock INNER JOIN producto p ON (stock.producto_id_producto = p.id_producto AND p.genero_producto_id_genero = 2) WHERE talle_producto_id_talle = :idMarca GROUP BY producto_id_producto", nativeQuery = true)
    public List<Stock> buscarMarcasMujeres(@Param ("idMarca") Long idMarca);
    
    @Query(value = "SELECT stock.* FROM stock INNER JOIN producto p ON (stock.producto_id_producto = p.id_producto AND p.genero_producto_id_genero = 3) WHERE talle_producto_id_talle = :idMarca GROUP BY producto_id_producto", nativeQuery = true)
